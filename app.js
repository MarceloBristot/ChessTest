...
io.on("connection", function (socket) {
    ...
    socket.on("enviar mensagem", function (mensagem_enviada, callback) {
        //mensagem_enviada = "[ " + pegarDataAtual() + " ] " + socket.apelido + " diz: " + mensagem_enviada.msg;
        //refreshMatch(mensagem_enviada.msg);
        io.sockets.emit("atualizar mensagens", mensagem_enviada);
        io.sockets.emit("achaPeca", mensagem_enviada.msg);
        callback();
    });
...
function refreshMatch(mensagem_enviada) {
    try {
        var teste = mensagem_enviada.split(" ");
        var peca = "&#9817";
        if(teste[0].lenght == 3){
